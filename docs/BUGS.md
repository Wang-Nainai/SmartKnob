...
## BUG-044（Regression）data-ready 判定两次误改 —— 判定依据回归实测

- 现象：环境数据两次"读不到"（v1.0.0-17/v1.0.0-18），status 日志显示
  持续 `0x8006`。
- 根因：data_ready 判定按两个"错误依据"先后修改：
  1) `status != 0`：意图覆盖实测的 bit15(0x8000) 瞬态，反而把
     "测量进行中"误判为 ready，触发假读取 → I2C 超时 (0x103) 刷屏；
  2) `status & 0x0800`（"回归 datasheet"）：把模块实际就绪状态
     `0x8006`（低 11 位=0x006）全部拒掉，永远不读测量。
- 教训：**实测才是唯一权威**。该模块的 ready 响应 = 低 11 位非零
  （0x8006 的低 11 位=0x006），与 datasheet 写的 bit11 不一致 —— 与
  BUG-009（XPT2046 轴向）同类教训：datasheet 与实测不符时以实测为准。
  最初"一直不读"的两个真因（上电时序、命令间隔）已分别修复。
- 当前状态：判定已恢复 `(status & 0x7FF) != 0`（低 11 位非零），
...
## BUG-046（逻辑死角）data-ready 坏 CRC 不参与自愈判定

- 现象（逻辑推演发现）：`scd40_data_ready` 的 CRC 校验失败路径直接返回
  `ESP_ERR_INVALID_CRC`，不计入 `not_ready_count`——若传感器持续返回
  坏 CRC（硬件劣化/总线干扰），调用方 `!= ESP_OK` 即 continue，读取
  阶段的 `read_fail_streak` 也不累计，自愈永不触发（BUG-043b 同类死角
  的另一半：总线错误已计入，坏 CRC 漏了）。
- 修复方式：CRC 失败路径同样累计 `not_ready_count` 并走自愈判定
  （60s→180s 退避、成功清零），与总线错误路径一致。
- 当前状态：已修复（编译验证）。防回退：自愈覆盖必须穷尽"判定→读取"
  全链路的每一种失败形态（总线错误/坏 CRC/未就绪/读取失败），新增
  失败分支必须挂到自愈计数上。
...
33. "少选项(2~3 个)度量切换"类 on_rotate：每事件限切 ±1 格（取 steps
    符号），不按幅度累计；不叠加 pm_shake 脉冲（棘轮力反馈已是触觉
    反馈，额外脉冲在 UNBOUNDED_DETENTS 模式会推动电机自转形成正反馈，
    见 BUG-045）。pg_menu 等多项目焦点导航按幅度累计是不同语义。

...
static const char page_html[] =
    ...
    "<div class=\"tip\">\xE6\xAF\x8F 1 \xE5\x88\x86\xE9\x92\x9F\xE9\x87\x87\xE4\xB8\x80\xE4\xB8\xAA\xE7\x82\xB9\xEF\xBC\x8C\xE5\x8E\x86\xE5\x8F\xB2\xE5\xAD\x98\xE4\xBA\x8E\xE5\x86\x85\xE5\xAD\x98\xEF\xBC\x8C\xE9\x87\x8D\xE5\x90\xAF\xE5\x90\x8E\xE6\xB8\x85\xE7\xA9\xBA</div></div>"
    "<h2>电机 / 手感</h2>"
    "<div class=\"card\">"
    ...
    "function setEM(i){em=i;drawEnv()}"
    "function drawEnv(){fetch('/api/envhist').then(r=>r.json()).then(h=>{ED=h;"
    "const cv=document.getElementById('envcv'),c=cv.getContext('2d');"
    "const W=cv.width,H=cv.height;c.clearRect(0,0,W,H);"
    "const arr=(em==0?h.co2:em==1?h.temp:h.rh)||[];"
    "const nm=ECFG[em][0],dv=ECFG[em][1],col=ECFG[em][2];"
    "c.strokeStyle='#232a36';c.lineWidth=1;c.beginPath();"
    "for(let i=0;i<5;i++){c.moveTo(34,H*i/4+2);c.lineTo(W-2,H*i/4+2)}c.stroke();"
    "let lg=nm+' -';"
    "if(arr.length){"
    "let mn=Math.min(...arr),mx=Math.max(...arr);if(mx===mn)mx=mn+dv;"
    "const f=v=>(v/dv).toFixed(dv>1?1:0);"
    "c.fillStyle='#8b93a3';c.font='10px sans-serif';c.textAlign='left';"
    "c.fillText(f(mx),2,10);c.fillText(f((mn+mx)/2),2,H/2+3);c.fillText(f(mn),2,H-6);"
    "const X=i=>34+(arr.length>1?i/(arr.length-1)*(W-40):0);"
    ...
    "if(arr.length<40){c.fillStyle=col;arr.forEach((v,i)=>{c.beginPath();c.arc(X(i),Y(v),2.2,0,6.3);c.fill()})}"
    "const hrs=Math.round(arr.length*(ED.iv_s||60)/3600);"
    "c.fillStyle='#8b93a3';c.font='10px sans-serif';"
    "c.textAlign='left';c.fillText('-'+hrs+'h',34,H-2);"
    "c.textAlign='center';c.fillText('-'+Math.round(hrs/2)+'h',(34+W-2)/2,H-2);"
    "c.textAlign='right';c.fillText('\\u73b0\\u5728',W-2,H-2);"
    "const last=arr[arr.length-1];"
    "lg=nm+' \\u6700\\u65b0 '+(last/dv)+' \\u00b7 '+f(mn)+'~'+f(mx)+' \\u00b7 '+arr.length+'\\u70b9'"
    "}"
    "document.getElementById('envlg').textContent=lg;"
    "}).catch(()=>{})}"
